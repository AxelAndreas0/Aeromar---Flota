  ...
  return <div className="login-wrap">
    <div className="login-box">
      <div className="login-logo">
        ...
        <div>
          <div className="login-title">Aeromar Fleet</div>
        </div>
      </div>
      ...
      <p style={{fontSize:11,color:'var(--gray-400)',textAlign:'center',marginTop:16}}>
        Contraseña inicial: nombre de usuario antes del @
      </p>
    </div>
  </div>
}
...
export default function App(){
  ...
  const loadAll = useCallback(async()=>{
    ...
    const [rv,rc,rco,rt,rh,rm,rtc,rca,rch] = await Promise.all([
      ...
      supabase.from('gastos_taller').select('*').order('fecha_ingreso',{ascending:false}),
      supabase.from('habilitaciones').select('*').order('fecha_vencimiento'),
      supabase.from('mantenimientos').select('*').order('fecha_proximo'),
    ...
    if(rtc.data&&rtc.data.length>0) setTipoCambio(rtc.data[0])
    if(rca.data&&rca.data.length>0) setConfigAlertas(rca.data[0])
  ...
  // Habilitaciones vencidas o próximas
  habilitaciones.forEach(h=>{
    const d = diffDays(h.fecha_vencimiento)
    if(d<=0) alerts.push({type:'red',msg:`${h.vehiculo_nombre} — ${h.tipo} VENCIDA (${Math.abs(d)} días atrás)`})
    else if(d<=h.dias_alerta) alerts.push({type:d<=7?'red':d<=15?'orange':'amber',msg:`${h.vehiculo_nombre} — ${h.tipo} vence en ${d} días`})
  })
  ...
  mantenimientos.filter(m=>m.estado!=='Completado').forEach(m=>{
    ...
    else if(d<=m.dias_alerta) alerts.push({type:d<=7?'red':'amber',msg:`${m.vehiculo_nombre} — ${m.tipo} en ${d} días`})
  })

  // Combustible límite
  vehiculos.forEach(v=>{
    if(!v.limite_combustible||v.limite_combustible===0) return
    const pct = Math.round((v.credito_utilizado/v.limite_combustible)*100)
    if(pct>=configAlertas.combustible_rojo) alerts.push({type:'red',msg:`${v.nombre} — Combustible al ${pct}% del límite`})
    else if(pct>=configAlertas.combustible_naranja) alerts.push({type:'orange',msg:`${v.nombre} — Combustible al ${pct}% del límite`})
    else if(pct>=configAlertas.combustible_amarillo) alerts.push({type:'amber',msg:`${v.nombre} — Combustible al ${pct}% del límite`})
  })
  ...
  const fuelColor = (v,cfg)=>{
    if(!v.limite_combustible||v.limite_combustible===0) return 'fuel-verde'
    const pct = Math.round((v.credito_utilizado/v.limite_combustible)*100)
  ...
  const fuelPct = v => {
    if(!v.limite_combustible||v.limite_combustible===0) return 0
  ...
  const saveCombustible = async form => {
    ...
    if(error){notify('Error: '+error.message,'error');return}
    // update credito utilizado
    const veh = vehiculos.find(v=>v.id===form.vehiculo_id)
    if(veh){
      await supabase.from('vehiculos').update({credito_utilizado:(veh.credito_utilizado||0)+monto}).eq('id',form.vehiculo_id)
    }
    notify('Carga de combustible registrada')
    setModal(null)
  }
  ...
  const saveTaller = async form => {
    ...
    const {error} = await supabase.from('gastos_taller').insert([{
      ...form,
      monto_gs:parseInt(form.monto_gs)||0,
  ...
  const saveHabilitacion = async form => {
    ...
    const {error} = await supabase.from('habilitaciones').insert([{...form,dias_alerta:parseInt(form.dias_alerta)||30}])
    if(error){notify('Error: '+error.message,'error');return}
  ...
  const saveMantenimiento = async form => {
    ...
    const {error} = await supabase.from('mantenimientos').insert([{
      ...form,
      dias_alerta:parseInt(form.dias_alerta)||15,
      monto_gs:parseInt(form.monto_gs)||0,
      km_ultimo:parseInt(form.km_ultimo)||0,
      km_proximo:parseInt(form.km_proximo)||0,
    }])
    if(error){notify('Error: '+error.message,'error');return}
  ...
  const deleteRow = async(table,id,msg) => {
    if(!window.confirm(`¿Eliminar ${msg||'este registro'}?`)) return
  ...
  function ViajeForm(){
    ...
    const selVeh = id => {
      ...
      if(v) setF(p=>({...p,vehiculo_id:id,vehiculo_nombre:v.nombre,chofer:v.chofer_asignado||''}))
    }
    return <>
      ...
      <div className="form-actions">
        ...
        <button className="btn btn-primary" onClick={()=>saveViaje(f)}>✓ Registrar viaje</button>
      </div>
    </>
  }
  ...
  function CombustibleForm(){
    ...
    const selVeh = id => {
      const v = vehiculos.find(x=>x.id===id)
      if(v) setF(p=>({...p,vehiculo_id:id,vehiculo_nombre:v.nombre}))
    }
    return <>
      <div className="form-grid">
        <div><label className="flabel">Fecha *</label><input type="date" className="finput" value={f.fecha} onChange={e=>upd('fecha',e.target.value)}/></div>
      ...
      <div className="form-actions">
        <button className="btn" onClick={()=>setModal(null)}>Cancelar</button>
        <button className="btn btn-primary" onClick={()=>saveCombustible(f)}>✓ Registrar carga</button>
      </div>
    </>
  }
  ...
  function MantenimientoForm(){
    ...
    return <>
      <div className="form-grid">
        ...
        <div><label className="flabel">Km en último mantenimiento</label><input type="number" className="finput" placeholder="0" value={f.km_ultimo} onChange={e=>upd('km_ultimo',e.target.value)}/></div>
        ...
        <div><label className="flabel">Km próximo mantenimiento</label><input type="number" className="finput" placeholder="0" value={f.km_proximo} onChange={e=>upd('km_proximo',e.target.value)}/></div>
        <div><label className="flabel">Alertar con X días de anticipación</label><input type="number" className="finput" value={f.dias_alerta} onChange={e=>upd('dias_alerta',e.target.value)}/></div>
  ...
  return <div className="app">
    ...
    <header className="hdr">
      ...
      <div className="hdr-right">
        {tipoCambio&&<div className="hdr-tc">USD/Gs. {fmtGs(tipoCambio.usd_gs)} · BCP {tipoCambio.fecha}</div>}
        {alerts.length>0&&<div className="hdr-tc" style={{background:'rgba(220,38,38,.3)',color:'#fca5a5'}}>⚠ {alerts.length} alerta{alerts.length>1?'s':''}</div>}
    ...
    <div className="app-body">
      ...
      <div className="content">
        ...
        {!loading&&tab==='dashboard'&&<>
          <div className="ph">
            <div><div className="ph-title">Dashboard</div><div className="ph-sub">Resumen operativo · {MESES[fMes]} {fAnio}</div></div>
          ...
          {alerts.length>0&&<div className="alert-list">
            ...
            {alerts.length>5&&<div style={{fontSize:13,color:'var(--gray-500)',padding:'4px 0'}}>+{alerts.length-5} alertas más</div>}
          </div>}

          <div className="metrics">
            <Metric label="Viajes del mes" value={viajesMes.filter(v=>!v.es_interno).length}/>
            <Metric label="Facturado Gs." value={`₲ ${fmtGs(totalGs)}`} color="blue"/>
            <Metric label="Facturado USD" value={`$ ${fmtUsd(totalUsd)}`} color="green"/>
            <Metric label="Km recorridos" value={fmtGs(kmMes)} sub="este mes"/>
            <Metric label="Vehículos libres" value={libres} color="green" sub={`${enRuta} en ruta`}/>
            <Metric label="Combustible mes" value={`₲ ${fmtGs(totalCombustGs)}`} sub="costo total"/>
          </div>
          ...
          <div className="g2">
            <div className="card">
              ...
              <div className="card-body">
                {usoPorVehiculo.length===0?<Empty text="Sin viajes este mes"/>:usoPorVehiculo.map((d,i)=>(
                  <div key={i} className="bar-row">
                    ...
                    <span className="bar-num">{d.val} viajes</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="card">
              <div className="card-header"><div className="card-title">Estado de la flota</div></div>
              <div className="card-body">
                {vehiculos.map(v=>(
                  <div key={v.id} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 0',borderBottom:'1px solid var(--gray-100)'}}>
                    <StatusDot estado={v.estado}/>
                    <div style={{flex:1}}>
                      <div style={{fontSize:13,fontWeight:600}}>{v.nombre}</div>
                      <div style={{fontSize:11,color:'var(--gray-400)'}}>{v.chofer_asignado||'Sin chofer asignado'}</div>
                    </div>
                    <Badge text={v.estado}/>
                  </div>
                ))}
          ...
          <div className="card">
            <div className="card-header"><div className="card-title">Últimos viajes</div></div>
            <div className="tw">
              <table className="tbl">
                <thead><tr><th>Fecha</th><th>Vehículo</th><th>Chofer</th><th>Origen → Destino</th><th>Tipo</th><th>Gs.</th><th>USD</th><th>Estado</th></tr></thead>
                <tbody>
                  {viajes.slice(0,8).map(v=>(
                    <tr key={v.id}>
                      ...
                      <td>{v.precio_gs?`₲ ${fmtGs(v.precio_gs)}`:'-'}</td>
                      <td>{v.precio_usd?`$ ${fmtUsd(v.precio_usd)}`:'-'}</td>
                      <td><Badge text={v.estado}/></td>
                    </tr>
                  ))}
                  {viajes.length===0&&<tr><td colSpan={8}><Empty/></td></tr>}
                </tbody>
              </table>
            </div>
          </div>
        ...
        {!loading&&tab==='flota'&&<>
          <div className="ph">
            <div><div className="ph-title">Flota</div><div className="ph-sub">{vehiculos.length} vehículos registrados</div></div>
          </div>
          <div className="g4">
            {vehiculos.map(v=>{
              const pct = fuelPct(v)
              ...
              return <div key={v.id} className="vcard">
                ...
                {v.limite_combustible>0&&<div className="vcard-fuel">
                  <div className="fuel-label">
                    ...
                    <span>₲ {fmtGs(v.credito_utilizado)} / {fmtGs(v.limite_combustible)} ({pct}%)</span>
                  </div>
                  <div className="fuel-bar"><div className={`fuel-fill ${fc}`} style={{width:`${pct}%`}}/></div>
                </div>}
                {canEdit&&<div className="vcard-actions">
                  {ESTADOS_V.map(e=>(
                    <button key={e} className={`btn btn-xs ${v.estado===e?'btn-primary':''}`}
        ...
        {!loading&&tab==='viajes'&&<>
          ...
          <div className="metrics">
            ...
            <Metric label="Km totales" value={fmtGs(viajesFiltrados.reduce((a,v)=>a+(parseInt(v.km_recorridos)||0),0))}/>
          </div>
          <div className="card card-np">
            <div className="tw">
              <table className="tbl">
                <thead><tr><th>Fecha</th><th>Vehículo</th><th>Chofer</th><th>Origen → Destino</th><th>Tipo</th><th>Km</th><th>N° Viaje</th><th>Gs.</th><th>USD</th><th>Factura</th><th>Estado</th><th>Obs.</th>{canEdit&&<th></th>}</tr></thead>
                <tbody>
                  ...
                  {viajesFiltrados.map(v=>(
                    <tr key={v.id}>
                      ...
                      {canEdit&&<td><button className="btn btn-xs btn-danger" onClick={()=>deleteRow('viajes',v.id,'este viaje')}>✕</button></td>}
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {!loading&&tab==='combustible'&&<>
          ...
          <div className="g4" style={{marginBottom:16}}>
            {vehiculos.filter(v=>v.limite_combustible>0).map(v=>{
              const pct=fuelPct(v)
              const fc=fuelColor(v,configAlertas)
              return <div key={v.id} className="card" style={{padding:16}}>
                <div style={{fontWeight:700,marginBottom:4}}>{v.nombre}</div>
                <div style={{fontSize:12,color:'var(--gray-500)',marginBottom:8}}>Límite: ₲ {fmtGs(v.limite_combustible)}</div>
                <div className="fuel-bar" style={{height:10,marginBottom:4}}><div className={`fuel-fill ${fc}`} style={{width:`${pct}%`}}/></div>
                <div style={{display:'flex',justifyContent:'space-between',fontSize:11,color:'var(--gray-500)'}}>
                  <span>₲ {fmtGs(v.credito_utilizado)} utilizado</span>
                ...
                <tbody>
                  ...
                  {combustMes.map(c=>(
                    <tr key={c.id}>
                      ...
                      {canEdit&&<td><button className="btn btn-xs btn-danger" onClick={()=>deleteRow('combustible',c.id,'esta carga')}>✕</button></td>}
                    </tr>
                  ))}
                </tbody>
              </table>
                  ...
                  {taller.map(t=>(
                    <tr key={t.id}>
                      ...
                      {canEdit&&<td><div className="ra">
                        ...
                        <button className="btn btn-xs btn-danger" onClick={()=>deleteRow('gastos_taller',t.id,'este registro')}>✕</button>
                      </div></td>}
                    </tr>
                  ))}
                </tbody>
        ...
        {!loading&&tab==='habilitaciones'&&<>
          <div className="ph">
            <div><div className="ph-title">Habilitaciones y documentos</div><div className="ph-sub">Vencimientos y alertas por vehículo</div></div>
            <div className="ph-actions">
          ...
          <div className="card card-np">
            <div className="tw">
              <table className="tbl">
                ...
                <tbody>
                  {habilitaciones.length===0&&<tr><td colSpan={7}><Empty text="No hay habilitaciones cargadas. Agregá los documentos de cada vehículo."/></td></tr>}
                  {habilitaciones.map(h=>{
                    const d=diffDays(h.fecha_vencimiento)
                    const estado=d<=0?'Vencido':d<=7?'Crítico':d<=h.dias_alerta?'Por vencer':'Vigente'
                    return <tr key={h.id}>
                      ...
                      <td className="td-m">{h.dias_alerta} días antes</td>
                      <td><Badge text={d<=0?'Vencido':d<=h.dias_alerta?'Pendiente':'Confirmado'}/></td>
                      {canEdit&&<td><button className="btn btn-xs btn-danger" onClick={()=>deleteRow('habilitaciones',h.id,'esta habilitación')}>✕</button></td>}
                    </tr>
                  })}
                </tbody>
              </table>
        ...
        {!loading&&tab==='mantenimientos'&&<>
          ...
          <div className="card card-np">
            <div className="tw">
              <table className="tbl">
                <thead><tr><th>Vehículo</th><th>Tipo</th><th>Último</th><th>Próximo</th><th>Días restantes</th><th>Km próximo</th><th>Costo est. Gs.</th><th>Estado</th>{canEdit&&<th></th>}</tr></thead>
                <tbody>
                  ...
                  {mantenimientos.map(m=>{
                    ...
                    return <tr key={m.id}>
                      ...
                      {canEdit&&<td><div className="ra">
                        ...
                        <button className="btn btn-xs btn-danger" onClick={()=>deleteRow('mantenimientos',m.id,'este mantenimiento')}>✕</button>
                      </div></td>}
                    </tr>
                  })}
                </tbody>
        ...
        {!loading&&tab==='choferes'&&<>
          ...
          <div className="card card-np">
            <div className="tw">
              <table className="tbl">
                <thead><tr><th>Nombre</th><th>CI</th><th>Teléfono</th><th>Viajes {MESES[fMes]}</th><th>Km {MESES[fMes]}</th><th>Facturado Gs. {MESES[fMes]}</th></tr></thead>
                <tbody>
        ...
        {!loading&&tab==='reportes'&&<>
          ...
          <div className="metrics">
            <Metric label="Viajes del período" value={viajesMes.filter(v=>!v.es_interno).length}/>
            ...
            <Metric label="Combustible Gs." value={`₲ ${fmtGs(totalCombustGs)}`}/>
                ...
                <table className="tbl">
                  ...
                  <tbody>
                    ...
                    {viajesMes.length>0&&<tr style={{background:'var(--gray-50)',fontWeight:700}}>
                      <td>TOTAL</td>
                      <td>{viajesMes.filter(v=>!v.es_interno).length}</td>
                      <td>{fmtGs(kmMes)} km</td>
                      <td>₲ {fmtGs(totalGs)}</td>
